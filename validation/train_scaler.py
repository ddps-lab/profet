# profet: train scaler model for scaler validation
...
# 1. make condition by instance, dataset size and model (for model-validation)
# 2. aggregate in same workload, so calculate mean value of exp01~exp09
# 3. build latency dictionary that keys are model and batchsize
# 4. build list of batchsize and min-max normalized batch latency
# 5. convert list to numpy array for training, and train scaler model, save model.
def build_batchsize_scaler(merge_data, instance_name, val_model_name, dataset_size):
    condition = ((merge_data['instance_name'] == instance_name)&
                 (merge_data['dataset'] == dataset_size) &
                 (merge_data['model'] != val_model_name))
    ...
    Path(f'./model/scaler/{SCALER_MODE}/{instance_name[:2]}').mkdir(parents=True, exist_ok=True)
    predictor_name = f"./model/scaler/{SCALER_MODE}/{instance_name[:2]}/scaler_{instance_name}_{val_model_name}_{dataset_size}dataset_BSpred.pickle"
...
def build_dataset_scaler(merge_data, instance_name, val_model_name, batchsize):
    condition = ((merge_data['instance_name'] == instance_name)&
    ...
    Path(f'./model/scaler/{SCALER_MODE}/{instance_name[:2]}').mkdir(parents=True, exist_ok=True)
    predictor_name = f"./model/scaler/{SCALER_MODE}/{instance_name[:2]}/scaler_{instance_name}_{val_model_name}_{batchsize}batchsize_DSpred.pickle"
    pickle.dump(poly_model, open(predictor_name, 'wb'))    

# 02-1-1. Train Batchsize Scaler & Save Models
for instance_name in sorted(instance_list):
    for val_model_name in sorted(model_list):
        for dataset_size in sorted(dataset_list):
            build_batchsize_scaler(merge_data, instance_name, val_model_name, dataset_size)
...
for instance_name in sorted(instance_list):
    for val_model_name in sorted(model_list):
        for batchsize in sorted(batchsize_list):
            build_dataset_scaler(merge_data, instance_name, val_model_name, batchsize)
